...
var module = (function() {
    ...
    function _addToDomAllFlickrEntries(response){
        ...
        entries.forEach(function(entry) {
            ...
            link.text = UtilService.getValidString(entry.title[0]);

            authorTextElm = _createTextElement('Author:', UtilService.getValidString(entry.author[0].name[0]));
            dateTextElm = _createTextElement('Date Taken:', new Date(entry['dc:date.Taken'][0]).toDateString());

            if (response.authorHandler && UtilService.isFunction(response.authorHandler)) {
    ...
    function _createHtmlElement(elmName){
        var elm;
        if (UtilService.isString(elmName)) {
